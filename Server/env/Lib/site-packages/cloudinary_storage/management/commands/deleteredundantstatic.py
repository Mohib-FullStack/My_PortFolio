...
class Command(deleteorphanedmedia.Command):
    ...
    def process_file(self, file):
        return self.storage._remove_extension_for_non_raw_file(
            self.storage._prepend_prefix(file)
        )

    def handle(self, *args, **options):
        if self.storage.read_manifest() is None:
            raise CommandError(
                "Command requires staticfiles.json. Run collectstatic command first and try again."
            )
