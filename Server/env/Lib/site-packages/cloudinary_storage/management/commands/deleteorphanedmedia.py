...
class Command(BaseCommand):
    ...
    def get_needful_files(self):
        ...
        for model in self.models():
            ...
            if media_fields:
                ...
                model_uploaded_media = model.objects.exclude(
                    **exclude_options
                ).values_list(*media_fields)
    ...
    def get_exclude_paths(self):
        ...
        paths = [
            storage._prepend_prefix(path)
            for path in app_settings.EXCLUDE_DELETE_ORPHANED_MEDIA_PATHS
        ]
        return tuple(paths)
    ...
    def handle(self, *args, **options):
        ...
        if (
            self.no_input
            or input("If you are sure to delete them, please type 'yes': ") == "yes"
        ):
