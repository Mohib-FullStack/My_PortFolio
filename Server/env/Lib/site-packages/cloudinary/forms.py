...
class CloudinaryJsFileField(forms.Field):
    default_error_messages = {"required": _("No file selected!")}
...
class CloudinaryUnsignedJsFileField(CloudinaryJsFileField):
    def __init__(
        self, upload_preset, attrs=None, options=None, autosave=True, *args, **kwargs
    ):
        if attrs is None:
            attrs = {}
        if options is None:
...
class CloudinaryFileField(forms.FileField):
    my_default_error_messages = {"required": _("No file selected!")}
